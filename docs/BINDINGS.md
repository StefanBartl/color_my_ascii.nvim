...
| `BufWipeout`, `BufDelete`, `BufUnload` (once) | `ColorMyAsciiFenceOpen_<tbuf>` | Clean up `:Fence open`'s temp file/extmarks/session state, and take back the session's autocmds with their records in `lib.nvim` and the group |
