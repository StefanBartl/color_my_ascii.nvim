...
local FORMATTERS = {
  ...
  yaml = { "prettier", "--parser", "yaml" }, yml = { "prettier", "--parser", "yaml" },
  markdown = { "prettier", "--parser", "markdown" }, md = { "prettier", "--parser", "markdown" },
  go = { "gofmt" }, sh = { "shfmt", "-" }, bash = { "shfmt", "-" },
  rust = { "rustfmt", "--emit", "stdout" }, rs = { "rustfmt", "--emit", "stdout" },
}
...
function M.run(_argv)
  ...
  if not cmd then
    util.notify("no formatter configured for '" .. lang .. "' (see config.fence_format.formatters)", vim.log.levels.WARN)
    return
  end
  ...
  vim.system(vim.deepcopy(cmd), { stdin = input, text = true }, function(res)
    vim.schedule(function()
      ...
      if not sp[1] or not ep[1] or ep[1] < sp[1] then return end
