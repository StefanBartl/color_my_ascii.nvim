...
return function(H)
  ...
  export.run({ html_out, '--html' })
  ok(vim.fn.filereadable(html_out) == 1, 'export --html: file written')
  local html_content = table.concat(vim.fn.readfile(html_out), '\n')
  ok(html_content:find('<!DOCTYPE html>', 1, true) ~= nil, 'export --html: is an HTML document')
  ok(html_content:find('<style>', 1, true) ~= nil, 'export --html: has a stylesheet')
  ok(html_content:find('let a = 0;', 1, true) ~= nil, 'export --html: original content present')
