...
function M.setup(opts)
  ...
  if not ok then
    notify(
      string.format('color_my_ascii: Failed to initialize configuration: %s', err),
...
function M.setup_buffer(bufnr)
  ...
  if not success then
    notify(
      string.format('color_my_ascii: Initial highlighting failed: %s', err),
      vim.log.levels.WARN
    )
  end
...
function M.highlight_buffer(bufnr)
  ...
  if cache_hit then
    ...
    if type(cached_blocks) ~= "table" or vim.tbl_isempty(cached_blocks) then
      vim.notify(string.format("Cache for buffer %d is empty or invalid, skipping block highlighting", bufnr), vim.log.levels.WARN)
    else
      ...
      for _, block in ipairs(cached_blocks) do
        if type(block) == "table" then
          ...
          if not success then
            vim.notify(string.format("Failed to highlight block: %s", err), vim.log.levels.ERROR)
          end
        else
          vim.notify("Skipped invalid block in cache (not a table)", vim.log.levels.WARN)
        end
      end
    end
  ...
  for _, block in ipairs(blocks) do
    ...
    if not success then
      ...
      notify(
        string.format('color_my_ascii: Block highlighting error: %s', err),
        vim.log.levels.WARN
      )
    end
  end
  ...
  if not success then
    notify(
      string.format('color_my_ascii: Inline code highlighting error: %s', err),
      vim.log.levels.WARN
    )
  end
