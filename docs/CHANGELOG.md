...
### Added
- `color_my_ascii.highlight` (`api/highlight.lua`): public, stable API for reading the applied highlighting back out as data — `runs_for_block(bufnr, block)` for the painted spans of a fenced block, `attrs_for_group(group)` for a group's resolved attributes (colors as `"#rrggbb"`, styles as booleans, `link=` chains followed, unset attributes left nil). Both read-only and side-effect free; a block this plugin has not painted reports no groups at all, which is the consumer's cue to fall back. Neither computation is new — `highlight_export` has done both for `:Fence export --html` and `:Fence yank --ansi` all along — but they are now a declared surface rather than internals. First consumer: [mdview.nvim](https://github.com/StefanBartl/mdview.nvim)'s `browser.highlighter = "nvim"`, which paints its browser preview with exactly what the buffer shows. See [api.md](api.md#highlight-read-back-api-for-plugin-authors).
...
### Fixed
- Fixed `cache_manager.setup_auto_cleanup()` leaking a new `uv` timer on every call instead of replacing the previous one. Same root cause as the `groups`/`languages` re-glob entry below: `plugin/color_my_ascii.lua` calls `setup()` eagerly with defaults, then a lazy-manager `opts`-driven config call runs it again with the real options — by design, see `init.lua`'s own comment on that. Every other piece `setup()` re-registers (autocmds, the debounce-cleanup autocmd) already cleared its previous registration first; the cache-cleanup timer did not, so each additional `setup()` call (a second lazy-manager pass, or a config reload) left one more 30-second cleanup timer running for the rest of the session, all doing the same redundant sweep. `setup_auto_cleanup()` now stops and closes a still-running timer before starting a new one (wkdbook-Lua Checklists `PERF-82`).
